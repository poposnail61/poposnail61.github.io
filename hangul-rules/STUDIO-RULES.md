...
| 중성×받침종류 / 패딩 자모별 | 60 | 222 | 1428 | 20.5 | 157.3 |
| 계열×종성 / 패딩 자모별 | 95 | 222 | 1743 | 19.6 | 128.6 |
| **중성×종성 / 패딩 계열별** ← 제안하신 구조 | 371 | 21 | 3423 | 20.6 | 140.3 |
| 중성×종성 / 패딩 자모별 | 371 | 222 | 4227 | 12.0 | 127.6 |
| 중성×종성 / 패딩 음절별 (상한) | 371 | 5490 | 25299 | 4.0 | 62.0 |

**패딩을 자모별로 주는 쪽이 레이아웃을 쪼개는 쪽보다 압도적으로 싸다.**
토폴로지 12개 + 자모 패딩은 996 파라미터로 26.0을 낸다. 제안하신 구조(컬럼 371개 +
초성 패딩)는 3,423 파라미터로 20.6이다. **파라미터를 3.4배 써서 오차 21%를 얻는 셈이다.**

자모별 패딩이 결정적인 이유는, 초성만 크기가 다른 게 아니라 **중성·종성도 자기 고유
크기를 갖기 때문**이다(SPEC R3의 종성 사다리 ㄴ 266 ~ ㅎ 420). 초성에만 패딩을 주면
이걸 레이아웃이 떠안아야 해서 컬럼을 588개로 쪼개게 된다.

### 권하는 구조: 4레이어 중첩

제안하신 구조를 그대로 쓰되 **컬럼 레이아웃을 템플릿이 아니라 보정 규칙으로** 옮기면
훨씬 싸진다. 명세 §9.1의 규칙 계층과 그대로 맞는다.

| 레이어 | 대응 | 단위 | 파라미터 | 중앙값 | 셀 대비 |
|---|---|---|---:|---:|---:|
| **L1** split 값과 슬롯 | `CompositionTemplate` | 토폴로지 12 | 678 | 55.0 | 5.8% |
| **L2** 자모 기본 패딩 | `JamoVariant` 기본값 | 초성 19×계열 + 중성 21 + 종성 27 | 1,482 | 26.0 | 2.7% |
| **L3** 컬럼 보정 | `CorrectionRule` `combination` | (중성,종성) 371 | 6,586 | **9.0** | **0.9%** |
| **L4** 음절 보정 | exact glyph rule | 필요할 때만 | 61,110 | 0.0 | 0.0% |

L3까지 쓰면 **리프 박스 최대 코너 편차 중앙값 9 em**(셀 높이 949의 0.9%)이다.
L3는 L2 위에 **가산되는 패딩 delta**라 컬럼별로 템플릿을 새로 만들 필요가 없다
(엔진에는 `SlotRectDelta`로 넣는다).

### 각 레이어가 실제로 담는 것

- **L1** — 골격. `gap`/`overlap`이 계열을 갈라놓는 부분(§4)이 여기 있다
- **L2** — 자모의 고유 크기. 종성 사다리와 초성 등급(`낣`/`랇`)이 여기서 해결된다.
  222개 값이 오차를 절반으로 줄인다(55.0 → 26.0)
- **L3** — 문맥 보정. 같은 종성 ㄼ이라도 중성이 ㅏ일 때와 ㅘ일 때 몫이 다르다
- **L4** — 나머지. Noto는 음절별 최적화 폰트라 완전히 없앨 수는 없다

### 이 표의 한계

L3의 컬럼 delta는 **관측된 371컬럼**에만 있다. 나머지 217컬럼(주로 겹중성·겹종성
조합)은 리프 단위 분해가 안 돼 L2까지만 적용되고 오차가 26 em 수준으로 남는다.
음절 기준으로는 4,585 / 11,172다. 이 범위를 넓히려면 §9의 재구성 문제를 먼저
풀어야 한다.
